"""
CNN-Transformer Prediction Script
...
if __name__ == "__main__":
    ...
    # ---- Configuration ----
    MODEL_PATH         = "model/CNN_Transformer.pth"
    LABEL_ENCODER_PATH = "model/label_encoder.pkl"
    VIDEO_FOLDER       = "./data/rotated_videos_splash_lc_2"
    OUTPUT_FOLDER      = "./output/predictions_fixed_3"
    ...
    behavior_names_list = list(behavior_names)
    supported_idx   = behavior_names_list.index('supportedrearing')   if 'supportedrearing'   in behavior_names_list else None
    unsupported_idx = behavior_names_list.index('unsupportedrearing') if 'unsupportedrearing' in behavior_names_list else None
    ...
    for video_id, (frame_indices, preds) in per_video.items():
        ...
        if supported_idx is not None and unsupported_idx is not None:
            preds = apply_supported_rearing_merge(preds, supported_idx, unsupported_idx)
